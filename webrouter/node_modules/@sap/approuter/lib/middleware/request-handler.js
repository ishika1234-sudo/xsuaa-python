...
var sapStatisticsUtils = require('../utils/sap-statistics-utils');
...
module.exports = function requestHandler(req, res, next) {
  ...
  if (!req.internalUrl) {
    if (logger) {
      logger.error('Missing internal url for request url %s', req.url);
    }
    return next();
  }
...
function onResponse(requests, requestStats, next) {
  ...
  let logger = incomingRequest.loggingContext.getLogger('/request/outgoing');
