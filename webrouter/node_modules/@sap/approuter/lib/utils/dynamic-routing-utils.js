...
var configurationUtils = require('../../lib/utils/configuration-utils');
...
function getRemoteConfiguration(req, res, next) {
  ...
  if (isHTML5RepoExcludedApp(appKey)){
...
function isDynamicRouting () {
  let defaultEnvHTML5Repo = null;
  if (fs.existsSync('default-env.json')){
    try {
      let defaultEnvJson = JSON.parse(fs.readFileSync('default-env.json', 'utf8'));
      defaultEnvHTML5Repo = defaultEnvJson['VCAP_SERVICES'] && defaultEnvJson['VCAP_SERVICES']['html5-apps-repo'];
    } catch (err){
  ...
  return html5RepoCredentials || defaultEnvHTML5Repo ? true : false;
}
...
function isHTML5RepoExcludedApp(appKey) {
  let currentAsRegex;
